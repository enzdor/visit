...
export default function Navigation(){
	...
	return(
		...
		{ open
			...
			<AnimatedBox style={spring} sx={{width: '100vw', height: '100vh', position: 'absolute', top: '0', left: '0', display: 'flex', justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgba(235, 245, 233, 0.95)'}}>
			<Grid container>
				<Grid item xs={12} sm={5} sx={{display: 'flex', justifyContent: 'center', alignItems: 'center'}}>
					<Stack spacing={3} sx={{mx: '1rem'}}>
							<AnimatedTypography onMouseLeave={() => setImage('base')} onMouseEnter={() => setImage('eat')} style={springEat} variant="h4" sx={{cursor: 'pointer'}}>FOOD</AnimatedTypography>
							<AnimatedTypography onMouseLeave={() => setImage('base')} onMouseEnter={() => setImage('go')} style={springGo} variant="h4" sx={{cursor: 'pointer'}}>VISIT</AnimatedTypography>
							<AnimatedTypography onMouseLeave={() => setImage('base')} onMouseEnter={() => setImage('hotel')} style={springHotel} variant="h4" sx={{cursor: 'pointer'}}>HOTELS</AnimatedTypography>
					</Stack>
				</Grid>
				<Hidden smDown>
					<Grid item xs={7} >
